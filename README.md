...
- Любая Linux-машина в LAN с установленным `podman` ≥ 4.4 (для Quadlet)
  и сетевым доступом к роутеру. Тестировалось на Ubuntu Server 24.04 LTS,
  но подойдёт любой современный дистрибутив с актуальным podman.
...
Все переменные читаются из env-файла (`/etc/mihomo-webui.env`) при запуске.
...
3. **`fetch_geosite_list`** — для `rule_kind=GEOSITE` скачиваем
   ...
   и пустые/`#`-строки пропускаются). Для прочих `rule_kind`
   (`DOMAIN`/`SUFFIX`/`KEYWORD`/`GEOIP`) шаг помечается `ok` с сообщением
